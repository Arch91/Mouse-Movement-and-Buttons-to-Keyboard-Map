...
; Sleep, 5                ; Wait for 5 milli seconds.
 ...
 DeadZone:=5			; Change it to your preference.
...
if (x1<LastX-DeadZone) {
Send {Blind}{%ToLeft% up}
Send {Blind}{%ToRight% down}
LastX=%x1%
xChanged:=1
}
else if (x1>LastX+DeadZone) {
...
if (y1<LastY-DeadZone) {
Send {Blind}{%ToUp% down}
Send {Blind}{%ToDown% up}
LastY=%y1%
yChanged:=1
}
else if (y1>LastY+DeadZone) {
Send {Blind}{%ToUp% up}
...
Send {Blind}{%ToDown% up}
Send {Blind}{%ToUp% up}
LastX=683			; Center Co-Ordinates for Mouse Cursor.
LastY=384			; Change them if they are not the center.
MouseMove, LastX, LastY
}
else if (xChanged=0) {
Send {Blind}{%ToRight% up}
Send {Blind}{%ToLeft% up}
LastX=683			; Center Mouse Cursor in horizont
MouseMove, LastX, y1
}
else if (yChanged=0) {
Send {Blind}{%ToDown% up}
Send {Blind}{%ToUp% up}
LastY=384			; Center Mouse Cursor in vertical
MouseMove, x1, LastY
}
...
;ToolTip, %LastX% %LastY%
 ...
 ;if (ax<60){                  ; Press and Hold the Right key if Mouse Cursor is moving towards right.
 ;Send {Blind}{%ToRight% down}
 ;}
 ;if (ax>60){                  ; Press and Hold the Left key if Mouse Cursor is moving towards left.
 ;Send {Blind}{%ToLeft% down}
 ;}
 ;if (ay<60){                  ; Press and Hold the Up key if Mouse Cursor is moving up.
 ;Send {Blind}{%ToUp% down}
 ;}
 ;if (ay>60){                  ; Press and Hold the Down key if Mouse Cursor is moving down.
 ;Send {Blind}{%ToDown% down}
 ;}
